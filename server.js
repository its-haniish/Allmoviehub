...
app.get("/:slug", async (req, res) => {
    try {
        const slug=req.params.slug.trim(); // Trim whitespace if any
        if (!slug) {
            return res.redirect("/"); // Handle empty slug case
        }

        const movie=await Posts.findOne({ slug }).lean(); // Use lean() for performance
        ...
        console.error("Error fetching movie:", error.message); // Log the error for debugging
